...
int main() {
	...
	MyLinkedList *common = new MyLinkedList();

	for (int i = 0; i < 4; i++) common->addAtTail(0);
	for (int i = 0; i < 3; i++) ListA->addAtTail(i + 1);
	for (int i = 0; i < 5; i++) ListB->addAtTail(i + 1);

	ListA->gettail()->next = common->getVirtualHead()->next;
	ListB->gettail()->next = common->getVirtualHead()->next;

	ListA->print_list();
	ListB->print_list();

	cout << common->getIntersectionNode_hash(ListA->getVirtualHead()->next,
											 ListB->getVirtualHead()->next)
			->val;
	cout << common->getIntersectionNode_dual_point(ListA->getVirtualHead()->next,
											 ListB->getVirtualHead()->next)
			->val;
