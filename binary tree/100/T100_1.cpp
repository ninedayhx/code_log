...
public:
	bool isSameTree(TreeNode *p, TreeNode *q)
	{
		...
		preOrderTraversal(p, dat1);
		preOrderTraversal(q, dat2);
		for (auto i: dat1)
			cout << i << ' ';
		cout << '\n';
		for (auto j: dat2)
	 ...
	 * @param BT
	 */
	void preOrderTraversal(TreeNode *BT, vector<int> &data)
	{
		if (BT) {
			data.push_back(BT->val);
			preOrderTraversal(BT->left, data);
			preOrderTraversal(BT->right, data);
		}else {
			data.push_back(11111);
		}
	}
};
